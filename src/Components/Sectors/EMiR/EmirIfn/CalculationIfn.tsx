...
export const CalculatonIfn = ({
  calculation,
  removeCalculationField,
  updateDataForCalculation,
  updateCalibrationValue,
  updateCalculationMode,
  changeStatus,
  ...props
}: CalculatonIfnPropsType) => {
        ...
        <tbody>
          ...
          <tr>
            ...
            <td rowSpan={11}>
              {calculation.calibrationValue === ValueIfnEnum.omActiv && (
                <>
                  {calculation.standardValue
                    ...
                    .map((el) => (
                      <li
                        className={el.checked ? st.checked : st.li}
                        key={el.id}
                      >
                        <input
                          type="checkbox"
              ...
              {calculation.calibrationValue === ValueIfnEnum.om ||
                (calculation.calibrationValue === ValueIfnEnum.voltsAC && (
                  <>---</>
                ))}
            </td>
        ...
        <table className={st.calculationTable} border={1}>
          <tbody>
            <tr>
              <td
                className={""}
              >{`Входная величина, ${calculation.calibrationValue}`}</td>
              <td
                className={""}
              >{`Значение оценки Xi, ${calculation.calibrationValue}`}</td>
              <td
                className={""}
              >{`Интервал, ±${calculation.calibrationValue}`}</td>
              <td className={""}>Тип неопре- делен- ностей</td>
              <td className={""}>Распре- деление вероятностей</td>
              <td
                className={""}
              >{`Стан- дартная неопре- деленность, ${calculation.calibrationValue}`}</td>
              <td className={""}>Коэф- фици- енты чувстви- тельнос- ти, Ci</td>
              <td
                className={""}
              >{`Вклад неопре- деленности, ${calculation.calibrationValue}`}</td>
            ...
            <tr>
              ...
              <td className={""}>{calculation.uncertaintyStnadardError}</td>
              <td className={""}>
                {calculation.uncertaintyStanadardErrorPercent}
              </td>
            </tr>
        ...
        <table border={1} className={st.calculationResultTable}>
          <tbody>
            ...
            <tr>
              <td
                className={""}
              >{`${calculation.calibrationDot} ${calculation.calibrationValue}`}</td>
              <td className={""}>{`${calculation.mode} `}</td>
              <td
                className={""}
              >{`${calculation.calibrationMiddleValue}  ${calculation.calibrationValue}`}</td>
              <td
                className={""}
