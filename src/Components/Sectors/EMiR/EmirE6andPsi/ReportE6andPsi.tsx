...
import { addNewCalibrationFieldForE6andPsiTC, changeReportE6TitleTC, changeStandardCalibrationDateE6TC, removeCalibrationFieldE6TC, ReportE6Type, updateCalibrationObjectTypeE6TC, updateCalibrationValueE6TC, updateDaraForCalculationCalibrationE6TC, updateTestVoltageReportE6TC } from '../../../../Redux/E6Reducer';
...
export const ReportE6andPsi = () => {
  ...
  const [selectedValue, setSelectedValue] = useState<string>(report!.calibrationObjectType)
  ...
  const selectHandler = (reportId: string, event: ChangeEvent<HTMLSelectElement>) => {
    const newValue = event.target.value;
    setSelectedValue(newValue);
    dispatch(updateCalibrationObjectTypeE6TC({ reportId: reportId, calibrationObjectType: newValue }))
  }
  ...
  const onblurHandler = (reportId: string) => {
    if (value.trim() !== '') {
      dispatch(addNewCalibrationFieldForE6andPsiTC({ reportId: reportId, calculationId: v1(), dot: +value, }))
    }
    setValue('')
  }

  const changeReportTitleHandler = (id: string, key: string, parameter: string) => {
    dispatch(changeReportE6TitleTC({ reportId: id, key: key, parameter: parameter }))
  }

  const changeStandardDateHandler = (reportId: string, key: string, title: string, id: string) => {
    dispatch(changeStandardCalibrationDateE6TC({ reportId: reportId, key: key, parameter: title, id: id }))
  }

  const updateDataForCalculation = (reportId: string, calculationId: string, index: number, dot: number) => {
    dispatch(updateDaraForCalculationCalibrationE6TC({ reportId: reportId, calculationId: calculationId, index: index, dot: dot }))
  }
  const removeCalculationField = (reportId: string, id: string) => {
    dispatch(removeCalibrationFieldE6TC({ reportId: reportId, calculationId: id }))
  }
  const updateCalibrationValueHandler = (reportId: string, calculationId: string, calibrationValue: string) => {
    dispatch(updateCalibrationValueE6TC({ reportId: reportId, calculationId: calculationId, calibrationValue: calibrationValue }))
  }
  const updateTestVoltageHandler = (reportId: string, calculationId: string, testVoltage: string) => {
    dispatch(updateTestVoltageReportE6TC({reportId: reportId, calculationId: calculationId, testVoltage: testVoltage}))
  }
      ...
      <div className={st.page}>
        ...
        <div>
          <div className={stl.reportNumber}>ПРОТОКОЛ КАЛИБРОВКИ № {''}
            <EditableSpan title={report!.reportNumber} changeTitle={(title) => { changeReportTitleHandler(report.reportId, 'reportNumber', title) }} />
          </div >
          <table className={stl.table} border={1}>
            <tbody>
              <tr>
                ...
                <td> {`Мегаомметр   `}
                  <select value={selectedValue} onChange={(selectedValue) => { selectHandler(report.reportId, selectedValue) }}>
                    <option value='Е6-24' selected={true}>Е6-24</option>
        ...
        report!.calculation.map((el, i) => {
          ...
          return (
            <div key={i} className={st.page}>
              ...
              <CalculationE6andPsi calculation={el}
